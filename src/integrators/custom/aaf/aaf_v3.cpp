 ...
 public:
	 ...
	 Spectrum getAnalytic(const Point &ref, const Matrix3x3 &rotMat, const Matrix3x3 &ltcW2l, const Float amplitude, const Spectrum &diffuseComponent, const Spectrum &specularComponent)
	 {
		 ...
		 for (uint32_t i = 0; i < triangleCount; i++)
			 sum += triangleEmitters[i].getAnalytic(ref, rotMat, ltcW2l, amplitude, diffuseComponent, specularComponent);
...
class AAF3 : public Integrator {
public:
	AAF3(const Properties &props)
		: Integrator(props) {
	...
	AAF3(Stream *stream, InstanceManager *manager)
	...
	Spectrum brdfEval(const PrimaryRayData &prd, BSDFSamplingRecord &bRec, const Matrix3x3 &ltcW2l = GET_MAT3x3_IDENTITY, const Float ltcW2lDet = 1.0f, const Float amplitude = 1.0f) const
	{
		const BSDF *bsdf = ((prd.its)->getBSDF(*prd.primaryRay));
		if (bsdf->isDiffuse())
			return bsdf->eval(bRec);
		else {
			Spectrum diffuseComponent = bsdf->getDiffuseReflectance(*prd.its);
			Spectrum specularComponent = bsdf->getSpecularReflectance(*prd.its);

			return Analytic::approxBrdfEval(bRec, ltcW2l, ltcW2lDet, amplitude, specularComponent, diffuseComponent);
		}
	}
	...
	void collectSamples(RadianceQueryRecord &rRec, const PrimaryRayData &prd, PerPixelData &ppd) 
	{
		...
		for (uint32_t emitterIdx = 0; emitterIdx < emitterCount; emitterIdx++) {
				...
				for (size_t i = 0; i < nEmitterSamples; i++) {
					...
					BSDFSamplingRecord bRec(*prd.its, rotMat * dRec.d);
					/* Evaluate BSDF * cos(theta) */
					const Spectrum bsdfVal = brdfEval(prd, bRec, ltcW2l, ltcW2lDet, amplitude);
				...
				ppd.totalNumShadowSample[emitterIdx] += d1Count > 0 ? nEmitterSamples : 1;
				ppd.colorEmitter[emitterIdx] += d1Count > 0 ? hitCount : emitterSamplers[emitterIdx].getAnalytic(prd.its->p, rotMat, ltcW2l, amplitude, bsdf->getDiffuseReflectance(*prd.its), bsdf->getSpecularReflectance(*prd.its));
		}
	}
...
MTS_IMPLEMENT_CLASS_S(AAF3, false, Integrator)
MTS_EXPORT_PLUGIN(AAF3, "AAF3");
MTS_NAMESPACE_END
