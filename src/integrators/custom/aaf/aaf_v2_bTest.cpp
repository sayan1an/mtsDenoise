...
class AAF2t : public Integrator {
	bool groundTruth = false;
public:
	AAF2t(const Properties &props)
		: Integrator(props) {
	...
	AAF2t(Stream *stream, InstanceManager *manager)
	...
	size_t nEmitterSamples = 100; //intial samples
	...
	void collectSamples(RadianceQueryRecord &rRec, const PrimaryRayData &prd, PerPixelData &ppd) 
	{
		...
		for (uint32_t emitterIdx = 0; emitterIdx < emitterCount; emitterIdx++) {
				...
				for (size_t i = 0; i < nEmitterSamples; i++) {
					...
					if (!groundTruth && intersectObject && value.average() > 0) {
						avg_d1 += abs(dot(dRec.n, (dRec.p - prd.its->p)));
						Float d2 = abs(dot((dRec.p - its.p), dRec.n));
				...
				ppd.isOccluded = isOccluded;
				ppd.d1[emitterIdx] += d1Count > 0 ? avg_d1 / d1Count : 0;
				ppd.totalNumShadowSample[emitterIdx] += isOccluded ? 0 : nEmitterSamples;
				ppd.colorEmitter[emitterIdx] += isOccluded ? Spectrum(0.0f) : hitCount;
		}
	}
	...
	{	
		...
		for (uint32_t emitterIdx = 0; emitterIdx < emitterCount; emitterIdx++) {
				...
				if (!groundTruth && ppd.d2Max[emitterIdx] > 100 * std::numeric_limits<Float>::min()) {
					...
					//if (numSamples < 1)
						//std::cout << "Whats going on " << numSamples << std::endl;
					ppd.totalNumShadowSample[emitterIdx] += numAdaptiveSample;
					//ppd.d1[emitterIdx] = s2;
		...
		for (size_t k = 0; k < emitterCount; k++)
			ppd.colorEmitter[k] /= static_cast<Float>(ppd.totalNumShadowSample[k] + 1);
	}
	...
	void pBufferToImage(ref<Bitmap> &result, const PerPixelData *pBuffer, const Vector2i &cropSize, const uint32_t nEmitters) 
	{
		...
		for (size_t j = 0; j < cropSize.y; j++)
			for (size_t i = 0; i < cropSize.x; i++) {
				...
				// Visualize d1
				//throughputPix[currPix] = Spectrum(pData.d1[0] / 50);
				...
				//if (pData.isOccluded)
					//if (pData.d1[0] < pData.d2Min[0] || pData.d1[0] < pData.d2Max[0] || pData.d2Max[0] < pData.d2Min[0])
						//std::cout << "Problem " << pData.d1[0] << " " << pData.d2Min[0] << " " << pData.d2Max[0] << std::endl;
...
MTS_IMPLEMENT_CLASS_S(AAF2t, false, Integrator)
MTS_EXPORT_PLUGIN(AAF2t, "AAF2t");
MTS_NAMESPACE_END
